...
フロー（fair-value の AnalyzeStockUseCase を骨格に）:
  1. AI設定取得・有効チェック
  2. レート制限チェック
  3. 対象領域に紐づく案件コンテキストを収集
  4. プロンプト構築 → LLM 呼び出し
  5. ログ記録し DomainStatement を返す
"""
...
from apps.generation.application.dto import (
    AchievementContext,
    DomainContext,
    EngagementContext,
)
from apps.generation.application.services.llm_client_service import build_llm_client
from apps.generation.application.services.statement_prompt_builder import StatementPromptBuilderService
...
if TYPE_CHECKING:
    ...
    from apps.careers.domain.repositories import EngagementRepository, SupportDomainRepository
    from apps.generation.domain.repositories import (
        AiConfigRepository,
        AiGenerationLogRepository,
        DomainStatementRepository,
    )
...
class GenerateDomainStatementUseCase:
    def __init__(
        self,
        ...
        statement_repository: DomainStatementRepository,
        ...
        self._prompt_builder = StatementPromptBuilderService()

    def execute(self, user_id: int, domain_code: str, *, persist: bool = False) -> DomainStatementEntity:
        # 1. AI設定
        ...
        # 3. コンテキスト収集
        domain = self._domain_repo.find_by_code(domain_code)
        if domain is None or domain.id is None:
            raise StatementSourceNotFoundError(f"支援領域が見つかりません: {domain_code}")
        engagements = self._engagement_repo.find_by_domain(user_id, domain.id)
        if not engagements:
            raise StatementSourceNotFoundError(
                f"「{domain.name}」に紐づく案件がありません。案件に領域を紐づけてから生成してください。"
            )
        context = DomainContext(
            domain_code=domain.code,
            domain_name=domain.name,
            engagements=[self._to_engagement_context(e) for e in engagements],
        )
        ...
        statement = DomainStatementEntity(
            user_id=user_id,
            support_domain_id=domain.id,
            body=body,
            char_count=len(body),
            source_engagement_ids=[e.id for e in engagements if e.id is not None],
            ai_model=response.model,
        )
        if persist:
            statement = self._statement_repo.save(statement)
        return statement
